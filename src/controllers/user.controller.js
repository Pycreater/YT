...
const registerUser = asyncHandler(async (req, res) => {
  // get data from user (required) else throw message
  // check that data is valid - not empty
  // check if it already exists || error
...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "Current user fetched succeddfully."));
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!channel?.length) {
    throw new ApiError(404, "ChannelDoes not exist");
  }
